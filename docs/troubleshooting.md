...
## 카탈로그 메타데이터가 Coach 기록 경로에서 사라지고 백그라운드 활성화가 기록 DB와 경쟁했다

- **Symptom**: Today 기본 프로그램의 `plank`는 초 단위 운동이지만 Coach와 상세 운동 카드가 legacy `ExerciseRow`만 전달해 reps 로거로 열거나 최근 세트 반복 경로로 저장할 수 있었다. Coach의 `다르게` 경로는 `countingConvention`과 선택 중량 메타데이터도 잃었다. 첫 로컬 bundle 또는 원격 catalog 활성화는 별도 SQLite 연결에서 수백 개의 개별 INSERT를 실행했고, 피커를 열 때도 원격 갱신을 다시 시작해 foreground 기록과 write lock을 경쟁할 수 있었다. 모든 catalog layer가 실패한 최종 fallback은 bridge가 있는 frozen 32행까지 제거했다.
- **Cause**: `editIntentStore`의 new intent가 catalog selection 대신 row 하나만 보존했고, `useQuickLog.repeat`는 현재 reps candidate인지 최종 확인하지 않았다. 프로그램은 logger capability와 무관하게 duration slot을 노출했다. 정규화 cache writer는 row별 INSERT 구조였고 main SQLiteProvider connection에는 `busy_timeout`이 없었다. seed fallback은 opaque bridge와 frozen identity를 같은 bridged 집합으로 취급했다.
- **Fix**: Coach는 `CatalogExerciseSelection`을 비동기로 해석해 intent와 화면 로거에 그대로 전달하고, repeat는 현재 reps candidate만 허용한다. catalog가 없어도 legacy `plank`는 fail-closed이며 기본·template·저장 프로그램에서 현 reps 로거가 처리할 수 없는 slot을 제외했다. 검색과 QuickLog는 현재 locale 뒤에 English display/alias를 bounded fallback으로 유지한다. catalog 활성화는 exercise→bridge→normalized child FK 순서의 최대 900-bind multi-row INSERT로 바꾸고, main/secondary connection에 5초 busy timeout을 설정했다. Boot가 검증된 로컬 catalog를 splash 뒤에서 먼저 준비한 후에만 원격 refresh를 비차단으로 시작하며 picker-triggered refresh는 제거했다. 최종 seed fallback은 frozen·ad-hoc row를 보존하고 opaque bridge만 제외한다.
- **Commit**: 04641fc
...
<!-- override-trigger: cdc8cc766b80bff60aa47b4e4e1b583b37b337ce merge(coach): record integrated session ancestry — tree 불변 Git 이력 통합이라 허위 Symptom을 만들지 않았다. 내러티브는 content/logs/OverDrive/2026-07-16-integrated-superseded-worktrees.mdx에 기록했다. -->
<!-- override-trigger: 993f67aa98622cd698f7137d6dcfe723b8075587 merge(food): record integrated manual meal ancestry — tree 불변 Git 이력 통합이라 허위 Symptom을 만들지 않았다. 내러티브는 content/logs/OverDrive/2026-07-16-integrated-superseded-worktrees.mdx에 기록했다. -->
<!-- override-trigger: da021344e1cce79ec7283cd0214971272fe48638 merge(catalog): record superseded contract ancestry — tree 불변 Git 이력 통합이라 허위 Symptom을 만들지 않았다. 내러티브는 content/logs/OverDrive/2026-07-16-integrated-superseded-worktrees.mdx에 기록했다. -->
